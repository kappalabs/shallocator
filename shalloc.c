...
static int init_pool() {
	...
	LOG("Size of the first block is init_size = 2^%lu = %uB, diff=%lu\n", mp.init_size, pow2(mp.init_size), sbrk(0)-mp.base_addr);
	LOG("Maximum pool size is max_size = 2^%lu = %uB\n", mp.max_size, -1);
...
void shee(void *ptr) {
	...
	if (b != NULL && b->k_size == mp.pool_size-1 && buddy < b) {
		LOG("--## SHRINKING ##--\n");
...
int main(int argc, char **argv) {
	...
	type *p1, *p2, *p3;
	if ((p1 = (type *) shalloc(128 * sizeof(type))) == NULL) {
		err(2, "shalloc");
	}
